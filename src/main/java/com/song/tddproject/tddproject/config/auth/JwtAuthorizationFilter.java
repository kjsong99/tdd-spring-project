...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        LOGGER.info("Jwt Authorization Filter : doFilterInternal 진입");
        String header = request.getHeader(JwtProperties.HEADER_STRING);

        if (header == null || !header.startsWith(JwtProperties.TOKEN_PREFIX)) {
            LOGGER.info("Jwt Authorization Filter : header가 비어있거나 Bearer로 시작합니다 ");
            chain.doFilter(request, response);
            return;
        }
        LOGGER.info("Jwt Authorization Filter : header - {}", header);
        ...
        if (username != null) {
            ...
            LOGGER.info("Jwt Authorization Filter : principal detail = {}",principalDetails);
